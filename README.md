...
> 위 흐름도는 과거 학습 당시 실험과 2026-08-27~28 GitHub 재검증을 구분합니다. 
...
- **그럴듯한 피처라고 해서 실제로 도움이 되는 것은 아니다.** 처음에는 Cabin 정보가 있는 승객과 없는 승객 사이에 생존 차이가 있을 수 있다고 생각해 `CabinKnown`을 만들었다. 하지만 실제 교차검증에서는 HGB F1이 약 `0.7733`에서 `0.7662`로 낮아졌다. 이 경험을 통해 Feature Engineering은 변수를 많이 만드는 일이 아니라, **가설을 세우고 실제 검증 결과를 보고 유지할지 버릴지 판단하는 과정**이라는 것을 배웠다.

- **처음 모델 비교에서 가장 좋아 보인 모델이 최종 모델이 되지는 않을 수 있다.** 기본 피처에서는 XGBoost가 강했고, `FamilySize`, `IsAlone`, `Title`을 추가한 뒤에는 HGB가 F1에서 가장 높았다. 그런데 Random Forest를 튜닝하고 RF·HGB·XGB 각각의 OOF threshold까지 다시 비교하자 RF가 F1 `0.7852`로 가장 높았다. 즉, **초기 모델 순위만 보고 결론을 내리기보다 같은 평가 기준에서 튜닝과 threshold까지 포함해 다시 비교해야 한다**는 것을 경험했다.

- **분류에서 `0.5`는 무조건 써야 하는 정답이 아니다.** 모델은 생존 여부를 바로 `0`이나 `1`로 출력하는 것이 아니라 생존할 확률을 출력하고, 어느 확률부터 `1`로 볼지 결정하는 기준이 threshold다. 처음에는 `0.5`를 자연스럽게 사용했지만 실제로는 RF `0.46`, HGB `0.53`, XGB `0.42`처럼 모델마다 F1이 가장 높은 기준이 달랐다. 이를 통해 **모델을 학습하는 것과 최종 분류 기준을 정하는 것은 별개의 의사결정**이라는 점을 배웠고, threshold를 학습 데이터에 직접 맞추지 않고 OOF probability를 이용해 정하는 방법도 익혔다.

- **원본 컬럼을 그대로 쓰는 것보다 문제의 의미를 담은 피처로 다시 표현할 수 있다.** `SibSp`, `Parch`를 이용해 `FamilySize`를 만들고, 혼자 탑승했는지를 `IsAlone`으로 표현하고, 이름에서는 `Title`을 추출했다. 이 과정을 통해 Feature Engineering은 단순한 계산식 추가가 아니라 **원본 데이터에 숨어 있는 가족 관계나 사회적 역할 같은 맥락을 모델이 사용할 수 있는 형태로 바꾸는 작업**이라는 것을 이해했다.

- **컬럼 이름만 보고 값의 단위를 당연하게 해석하면 안 된다.** 처음에는 `Fare`를 승객 한 명의 개인 운임으로 생각했지만, 데이터를 살펴보면서 같은 `Ticket`을 공유하는 승객들에게 동일한 Fare가 반복되는 경우를 확인했다. 그래서 Fare가 개인 단위 값이라기보다 동행 그룹의 예약·티켓 단위 운임 성격을 가질 수 있다고 보고, 같은 Ticket을 공유하는 승객 수인 `TicketGroupSize`로 나눈 `FarePerPerson`을 만들었다. 이 경험을 통해 **Feature Engineering 전에 컬럼의 이름뿐 아니라 실제 값이 어떤 단위와 구조로 기록되어 있는지 확인하는 것이 중요하다**는 것을 배웠다.

- **결측값도 다른 변수의 맥락을 이용해 더 합리적으로 채울 수 있다.** 당시 학습 과정에서는 `Age` 결측치를 전체 승객의 중앙값 하나로 채우는 대신, 이름에서 추출한 `Title`별 연령 분포를 활용해 보정하는 실험을 했다. `Master`, `Miss`, `Mr`, `Mrs`처럼 호칭마다 주로 나타나는 연령대가 다를 수 있기 때문이다. 이 경험을 통해 **결측치 처리는 단순히 빈 값을 채우는 작업이 아니라 변수 사이의 관계를 이용해 더 타당한 값을 추정하는 과정이 될 수 있다**는 것을 배웠다. 다만 이 방식은 당시 학습에서 시도한 실험이며, 현재 GitHub 재검증 파이프라인에서는 Fold별 median imputer를 사용한다.

### GitHub로 다시 정리하며 확인한 점

- **좋은 피처 아이디어도 계산 방식이 잘못되면 검증 결과를 믿기 어렵다.** `TicketGroupSize`는 같은 Ticket을 사용하는 승객 수를 이용하는 피처라서, 교차검증 전에 전체 데이터의 Ticket 개수를 계산해버리면 validation Fold의 정보가 학습 과정에 섞일 수 있다. 그래서 GitHub 정리 과정에서는 각 학습 Fold 안에서만 Ticket 통계를 계산하도록 다시 구현했다. 이를 통해 **데이터 누수는 target을 직접 사용하는 경우에만 생기는 것이 아니라 전처리나 파생 피처를 만드는 과정에서도 발생할 수 있다**는 점을 확인했다.

- **hold-out은 성능을 더 높이기 위한 데이터가 아니라 마지막에 일반화 성능을 확인하기 위한 데이터다.** 개발 데이터의 OOF F1은 `0.791`이었지만 마지막 hold-out F1은 `0.766`으로 더 낮았다. 결과를 본 뒤 threshold나 모델을 다시 바꾸면 점수를 올릴 수도 있었지만 그렇게 하지 않았다. hold-out 결과를 보고 계속 수정하기 시작하면 그 데이터도 사실상 학습과 의사결정에 사용한 데이터가 되기 때문이다. 이 과정을 통해 **마지막 평가 데이터는 결과가 기대보다 낮더라도 독립성을 지키는 것이 중요하다**는 점을 이해했다.

- **좋은 결과를 보여주는 것만큼 그 결과를 어디까지 믿을 수 있는지 설명하는 것도 중요하다.** 현재 Ticket 피처는 Fold 내부에서 계산하도록 개선했지만, 같은 Ticket을 가진 승객이 서로 다른 Fold에 들어갈 수 있다는 구조적 한계는 남아 있다. 또한 Ticket 피처를 적용한 RF 결과는 확인했지만 같은 조건에서 HGB와 XGB를 다시 비교한 것은 아니다. 그래서 `RF가 모든 모델 중 최고다`라고 단정하지 않고, **실제로 검증한 범위와 아직 확인하지 않은 범위를 구분해서 기록하는 것이 더 정확한 결과 해석**이라는 점을 배웠다.

...
- **좋아 보이는 피처가 실제로 좋은 피처인 것은 아니다.** `CabinKnown`은 객실 정보의 존재 여부가 의미 있는 신호일 것이라는 가설로 만들었지만, HGB F1이 약 `0.7733 → 0.7662`로 낮아져 제외했다. Feature Engineering은 변수를 많이 만드는 것이 아니라 가설을 만들고 검증해 유지 여부를 판단하는 과정임을 배웠다.
- **첫 모델 비교 결과가 최종 모델 선택을 결정하지 않는다.** 기본 피처에서는 XGBoost가 강했고 맥락 피처 추가 후에는 HGB가 F1에서 앞섰지만, RF 튜닝과 모델별 OOF threshold 비교까지 진행한 뒤에는 RF가 F1 `0.7852`로 당시 최종 후보가 됐다.
- **분류 threshold `0.5`도 고정된 정답이 아니다.** 모델이 출력한 probability를 실제 class로 바꾸는 기준 역시 평가 지표와 목적에 따라 선택할 수 있으며, 학습 데이터에 직접 맞추지 않고 OOF probability로 threshold를 정하는 방법을 익혔다.
- **원본 데이터를 문제의 맥락으로 다시 표현하는 것이 중요하다.** `FamilySize`, `IsAlone`, `Title`을 만들면서 단순한 컬럼 자체보다 가족 구성이나 사회적 역할처럼 문제와 연결된 정보를 피처로 표현하는 과정을 경험했다.

### GitHub로 다시 정리하며 확인한 점

- **좋은 피처 아이디어도 검증 구현이 잘못되면 결과를 신뢰하기 어렵다.** Ticket 그룹 정보를 전체 데이터에서 미리 계산하지 않고 각 학습 Fold 내부에서 계산하도록 바꾸면서, 전처리와 파생 피처 생성 단계에서도 데이터 누수를 고려해야 한다는 점을 확인했다.
- **hold-out은 점수를 높이기 위한 데이터가 아니라 마지막 일반화 확인용 데이터다.** OOF F1 `0.791`보다 hold-out F1 `0.766`이 낮았지만, 그 결과를 보고 모델이나 threshold를 다시 수정하지 않았다. 평가 데이터를 반복해서 의사결정에 사용하면 더 이상 독립적인 평가가 아니라는 점을 명확히 이해하게 됐다.
- **성능 수치와 함께 결과를 어디까지 믿을 수 있는지도 기록해야 한다.** 같은 Ticket을 가진 승객이 서로 다른 Fold에 들어갈 수 있다는 한계와 Ticket 피처 조건에서 HGB/XGB까지 다시 비교하지 않았다는 범위를 남기면서, 높은 점수만 제시하는 것보다 검증 범위와 한계를 함께 설명하는 것이 중요하다고 판단했다.

...
### 결과
GitHub 정리 과정의 재구성 실험에서 튜닝 RF + 맥락 피처 + Ticket 피처의 OOF threshold 최적값은 `0.47`이었다.
...
### 판단
실제로 수행한 실험 중 가장 높은 OOF F1을 기록해 최종 후보로 사용했다.

### 표현상의 주의
Ticket 피처를 적용한 HGB/XGB를 동일 조건으로 새로 비교한 것은 아니다. 따라서 “모든 가능한 모델 중 최적”이라고 표현하지 않고 **“실제로 수행한 실험 중 가장 높은 결과”**라고 기록한다.
...
위 항목은 **이 저장소에서 수행한 실험이 아니라**, 프로젝트를 정리하면서 확인한 후속 검토 아이디어다. 현재 기록의 목적은 이들을 새로 수행하는 것이 아니라 당시 학습과 실제 재검증 범위를 정확히 남기는 데 있다.
